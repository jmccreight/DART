...
domain_ens_size = len(tuple(domainPath.glob(pattern)))
...
forcing_ens_size = len(tuple(forcingPath.glob(pattern)))
...
forcing_ens_files = sorted([str(d) for d in list(forcingPath.glob(pattern))] * domain_ens_size)
...
soil_ens_files=[str(d) + '/soil_properties.nc' for d in sorted(domain_ens)] * forcing_ens_size
